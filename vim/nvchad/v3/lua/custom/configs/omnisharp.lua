...
local function parse_solution(sln_path)
  ...
  for line in file:lines() do
    ...
    -- Extract: name = "MyProject", rel_path = "src\MyProject\MyProject.csproj"
    local name, rel_path = line:match('Project%("[^"]*"%)%s*=%s*"([^"]+)"%s*,%s*"([^"]+%.csproj)"')
...
-- Show popup picker to select and load a project
function M.load_project_picker()
  local file_path, file_type = find_solution_or_project()
